...
def read_coverage_mam_score(mams, overlap_threshold = 20):
    ...
    for j in range(len(T)):
        ...
        if I_values:
            ...
            I_values_plus_chord_diff = [ (j_prime, c_val + (v.val - (mams[j_prime].d - v.c + 1 ) - 0.0001 if v.x != mams[j_prime].y else v.val - 0.0001)) for j_prime, c_val in I_values]  # Penalize an extra 0.1 to prefer non-overlapping solutions in case of tie breakers
