...
def find_nams_strobemap(outfolder, read_path, refs_path, out_path, nr_cores, min_mem):
    ...
    try: # slaMEM throws error if no MEMs are found in any of the sequences
        subprocess.check_call([ 'StrobeMap', '-n' , "2", '-k' , '15', '-v' , "16", '-w' , "45", '-t', str(nr_cores), '-s', '-o', outfolder, refs_path, read_path ], stdout=stdout_file, stderr=stderr_file)
        print("Using StrobeMap")
        print([ 'StrobeMap', '-n' , "2", '-k' , '15', '-v' , "16", '-w' , "45", '-t', str(nr_cores), '-s', '-o', outfolder, refs_path, read_path ])
    except:
